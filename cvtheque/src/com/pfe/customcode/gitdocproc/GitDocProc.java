package com.pfe.customcode.gitdocproc;
...
@CVComponentConfigClass(configClass = GitDocProcConfig.class)
@CVComponentDescription(value = "git docproc")
public class GitDocProc extends CustomDocumentProcessor implements CVComponent {

	private Logger logger = Logger.getLogger(GitDocProc.class);
	public GitDocProc(CVComponentConfig config) throws Exception {
		super(config);
		// TODO Auto-generated constructor stub
	}

	@Override
	public void process(DocumentProcessingContext arg0, ProcessableDocument document) throws Exception {
		String login = document.getFirstMetaValue("login_git");
		int followers = 0;
		int following = 0;
		int nbre = 0;
		String link = "";
		...
		try {
			...
			logger.info("error in git !!! ");
		}
		document.addMeta("gitfollowers", Integer.toString(followers));
		document.addMeta("gitfollowing", Integer.toString(following));
		document.addMeta("gitnbre", Integer.toString(nbre));
		document.addMeta("gitlink", link);
		int i=0;
	      while (i<titles.size()) {
	      ...
	      document.addCustomDirective("datamodel_class", "git");
